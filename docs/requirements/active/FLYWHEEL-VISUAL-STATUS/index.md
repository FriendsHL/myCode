...
## 2026-05-19 启动 — observability framing

用户 confirm: "类似于飞轮的可观测"。从 status panel 升级为 observability panel：
- 4 类节点 (AUTO / USER / HYBRID / ENTRY)
- 每节点 4 维 metric (in-flight / last activity / lag / recent error / today aggregate)
- 健康颜色 encoding (healthy / warn / stale / dormant / empty)
- 顶部双 tab (agentType × surface)，surface tab 内单 surface 纵向 timeline
- panel 内只读，USER gate 节点 chip 跳现有 operate page
- 默认 0 BE 改动（FE 多 useQuery + 聚合），endpoint 不支持时 FE-Dev 取证后 flag

详见 [prd.md "2026-05-19 启动"](prd.md) section。

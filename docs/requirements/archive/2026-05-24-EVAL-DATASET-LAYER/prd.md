...
- ⚠️ **skill surface dataset BE 已 ready / FE UI 留 V2** (Phase 2 r4-r2 scope expansion ratify) — 见下"r4-r2 scope 扩展"段
- ❌ behavior_rule surface dataset（schema + service + FE 全 V2）

### r4-r2 scope 扩展 (2026-05-24 ratify)

Phase 3 Reviewer 升级 SkillAb 路径 silent-failure 为 blocker (team-lead 决策)，r4-r2 round 实际扩展 V1 范围到**部分 skill surface 支持**：

| 组件 | V1 状态 | 说明 |
|---|---|---|
| **V113** migration (`t_skill_ab_run.dataset_version_id`) | ✅ V1 实施 | 跟 V111 schema parity, FK→t_eval_dataset_version, nullable, partial index |
| **SkillAbRunEntity.datasetVersionId** field | ✅ V1 实施 | mirrors PromptAbRunEntity.datasetVersionId |
| **SkillController.startAbTest body.datasetVersionId** | ✅ V1 实施 | 接受 + 透传给 service, blank-string normalisation |
| **SkillAbEvalService.createAndTrigger 6-arg overload** | ✅ V1 实施 | 5-arg legacy delegate to 6-arg with null, 现有 caller 全保留兼容 |
| **SkillController.toAbRunMap emit datasetVersionId** | ✅ V1 实施 | 4 个 caller (startAbTest/getAbTest/listAbTests/listAbTestsGlobal) 自动得到字段 |
| **FE SkillAbPanel dataset selector UI** | ❌ **V2** | 当前 SkillAbPanel.tsx 仅含 scope 注释，**不暴露 user 入口** |
| **FE 类型 `SkillAbRun.datasetVersionId/Label` + `StartAbTestRequest.datasetVersionId`** | ✅ V1 实施 (optional/forward-compat) | V1 BE 不 emit/不接 → undefined / no-op；V2 加 UI 时 FE 零改动 pickup |
| **BehaviorRuleAbRun 全链路** | ❌ **V2** | PRD 同条排除 |

**为什么这样切**：silent-failure path 的"FE 选了 dataset 但 BE 丢"问题在 V1 不会触发（FE 没 UI 让 user 选），但**BE 先 ready** 让 V2 真加 FE UI 时无需重做 schema + service。代价：V1 commit 含部分 V2 前置工作，commit message 显式标注。

**对 V1 行为零影响**：V1 caller 调 skill A/B 不传 datasetVersionId → service 走 null path → legacy 行为完全不变。

...
- **Phase 1.4** ✅ Dashboard behavior_rule panel + ProposeOptimizationTool 放开 behavior_rule + CanaryRolloutService 真接 behavior_rule canary + BehaviorRuleVersionController REST endpoint（2026-05-15，1670 test BUILD SUCCESS = 1662 baseline + 4 BE-1/2 + 4 BE-3 controller；零行为漂移；零核心 7+1 文件 diff + 零核心 FE 3 文件 diff）。落地形态：(a) BE-1 `ProposeOptimizationTool.ALLOWED_SURFACES` 加 SURFACE_BEHAVIOR_RULE + schema enum 扩，curator agent 真能发 V4 surface proposal；(b) BE-2 `CanaryRolloutService.normalizeSurfaceType` 接受 {'skill', 'behavior_rule'}；startCanary / publish / rollback 三路径加 behavior_rule 分支（注 BehaviorRuleVersionRepository + BehaviorRulePromotionService 无 DI cycle 不需 @Lazy），publish 调 `BehaviorRulePromotionService.promote(versionId)` 而非 `SkillAbEvalService.promoteCandidate`；rollback 仅翻 canary row 不动 BehaviorRuleVersion.status (保审计)；defensive agentId cross-check 防 attribution mislabeling；(c) BE-3 新加 `BehaviorRuleVersionController` (`/api/behavior-rules/versions?agentId&status` + `/{id}`) + `BehaviorRuleVersionResponse` DTO 11 字段 + `findByAgentIdAndStatusOrderByVersionNumberDesc` derived query；(d) FE 设计选择 (B') —— BehaviorRuleEvolutionPanel 嵌入 `Insights.tsx` 第 3 tab（与 OptimizationEvents 同构，不新增路由），新建 `api/behaviorRules.ts` + `pages/BehaviorRuleEvolution.tsx` + `components/behaviorRules/BehaviorRuleEvolutionPanel.tsx`；CanaryPanel.tsx 加 optional surfaceType + entityNoun props (additive backwards-compat，V2 SkillAbPanel 行为不变)；queryKey 加 surfaceType 维度避不同 surface 缓存撞车；(e) 测试新增 8 case：ProposeOptimizationToolTest +1 / CanaryRolloutServiceTest +4 (1 rename + 3 new) / BehaviorRuleVersionControllerTest 4
- **Phase Final** ✅ 归档（2026-05-15）—— mvn 1670/0/0/84 BUILD SUCCESS + tsc + npm build 三件套已在 Phase 1.4 commit `7d3c2ec` 内独立 verify；核心 7+1 文件 + 核心 FE 3 文件 git diff = 0 全程守住 Iron Law；需求包 `active/MULTI-SURFACE-FLYWHEEL/` → `archive/2026-05-15-MULTI-SURFACE-FLYWHEEL/`；delivery-index.md 新增交付行（3 commit 链 `6da1801` Phase 1.1 → `9cd74d8` Phase 1.2+1.3 → `7d3c2ec` Phase 1.4）。**e2e cron live 验证**（attribution-curator 真发 behavior_rule proposal → operator approve → BehaviorRuleImproverService 生成 candidate → A/B → canary → promote）留 dogfood 期数据沉淀，需用户在 dashboard 真触发；本期不阻塞归档。
